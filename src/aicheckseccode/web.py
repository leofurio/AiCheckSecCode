...
def render_report_page(report_id: str, source: str, report) -> str:
    ...
    findings_rows = "\n".join(
        f"<tr><td>{html.escape(finding.rule_id)}</td><td>{html.escape(finding.severity.value)}</td><td>{html.escape(finding.path or '')}</td><td>{finding.line or ''}</td><td>{html.escape(finding.title)}</td><td>{html.escape(finding.message)}</td></tr>"
        for finding in report.findings
    ) or '<tr><td colspan="6">No findings detected.</td></tr>'
